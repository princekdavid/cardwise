...
class RecommendationEngineTest {
    ...
    @Test fun ignoresWrongCategory() {
        val result = RecommendationEngine.recommend(PaymentContext("Travel", 1_000.0), listOf(card(1)), mapOf(1L to listOf(RewardRule("Dining", 10.0))))
    ...
    @Test fun ignoresZeroRewardAndZeroAmountRecommendations() {
        assertTrue(RecommendationEngine.recommend(PaymentContext("Dining", 0.0), listOf(card(1)), mapOf(1L to listOf(RewardRule("Dining", 10.0)))).isEmpty())
    }
    ...
    @Test fun evaluationTraceIsNoMatchWhenNoRuleApplies() {
        ...
        assertEquals(RecommendationTraceOutcome.NO_MATCH, trace.outcome)
        assertTrue(trace.steps.last().detail.contains("no eligible recommendation"))
    }

    @Test fun invalidEvaluationProducesPendingDownstreamSteps() {
        val trace = RecommendationEngine.evaluate(PaymentContext(" ", 0.0), emptyList(), emptyMap()).trace
